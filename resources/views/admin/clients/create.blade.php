...
@section('content')
<x-breadcrumb :links="['Clientes' => route('admin.clients.index'), 'Registrar' => '']" />
<div class="mb-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-800">Registrar Cliente</h1>
...
<div class="bg-white rounded-lg shadow overflow-hidden max-w-4xl">
    <form action="{{ route('admin.clients.store') }}" method="POST" class="p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <!-- Estado de Membresía -->
            <div>
                <label for="membership_status" class="block text-sm font-medium text-gray-700">Estado Inicial de Membresía *</label>
                <select name="membership_status" id="membership_status" required
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    <option value="sin_membresia" {{ old('membership_status') == 'sin_membresia' ? 'selected' : '' }}>Sin Membresía</option>
                    <option value="activo" {{ old('membership_status') == 'activo' ? 'selected' : '' }}>Activo</option>
                    <option value="vencido" {{ old('membership_status') == 'vencido' ? 'selected' : '' }}>Vencido</option>
                </select>
            </div>
        </div>
